...
// bytes that dex.DeriveAssetID asserts in luxfi/dex pkg/dex (assetid_test.go,
...
// registered AssetID (here) and a swap-derived AssetID (dex, via the 0x9999 resolver)
...
//
// These vectors are BYTE-FOR-BYTE the dex vectors: networkID=2, source chain id all
...
// vectors are generated against — identical to dex's chainAllOnes().
...
func TestAssetID_GoldenKAT_MatchesDexcore(t *testing.T) {
	...
	vectors := []struct {
		...
		kind AssetKind
		ref  []byte
		want string // BYTE-IDENTICAL to dex.AssetIDGoldenVectors
	}{
	...
	for _, v := range vectors {
		...
		if h := hex.EncodeToString(got[:]); h != v.want {
			t.Fatalf("%s: registry AssetID diverged from the dex golden KAT:\n got  %s\n want %s", v.name, h, v.want)
		}
	}
}
