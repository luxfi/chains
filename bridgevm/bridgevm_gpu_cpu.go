...
func cpuSignerApply(
	...
	for i := range ops {
		...
		case signerOpKindOptIn:
			...
			copy(s.CoronaPubKey[:], op.CoronaPubKey[:])
	...
	for i := range ops {
		...
		case liquidityOpKindAccrueFee:
			...
			for j := range liquidity {
				...
				// __uint128_t; Go's math/bits.Mul64 gives the same 128-bit
				// product without an external dep.
				deltaHi, deltaLo := mulDiv64(feeLo, s.AmountLo, poolLo)
				_ = deltaHi // 128-bit product / 64-bit divisor → 64-bit quotient
...
// and the result is asserted to fit 64 bits by the caller. Mirrors the C++
// `__uint128_t prod = (__uint128_t)fee_lo * s.amount_lo; delta = prod / pool_lo`.
//
// Returns (hi, lo) of the quotient; in this op the hi limb is always 0
// because the caller has already gated on the saturating conditions
// (poolTotal.hi == 0, feeTotal.hi == 0, s.AmountHi == 0).
func mulDiv64(a, b, d uint64) (uint64, uint64) {
	...
	hi := p3 + (mid >> 32) + (p2 >> 32)
	// Bit-by-bit long division — the C++ relies on `__uint128_t / uint64_t`;
	// Go has no native 128/64. The bit shift is fine here because callers gate
	// by `feeTotal.hi == 0 && s.AmountHi == 0`, so `hi` is always 0 in
	// practice. We still implement the long division for correctness if
	// fixture inputs push past that bound in the future.
	if hi == 0 {
		return 0, lo / d
	}
	var quo uint64
	...
	for i := 127; i >= 0; i-- {
		...
		if rem >= d {
			rem -= d
			if i >= 64 {
				// quotient bits land in hi; but since callers gate hi=0 for
				// the result, we drop them — matches the saturating contract.
			} else {
				quo |= uint64(1) << uint(i)
			}
		}
...
func computeSignerSetRoot(signers []Signer, out []byte) (uint32, uint32, uint32, u128) {
	...
	for i := range signers {
		...
		o += 48
		copy(leaf[o:o+32], s.CoronaPubKey[:])
		o += 32
