...
//
// The binary's FILENAME must be the CB58 of relayvm.VMID
// (sP6dLqrrBR9w3soP18fbJ3YzZecZdD7DDdfH2cFhhLq7Hy9bz) — that is how the node's
...
func run(ctx context.Context, args []string, out, errOut io.Writer) int {
	// Built through the same vms.Factory the node's registry uses. relayvm
...
func fail(w io.Writer, err error) int {
	fmt.Fprintf(w, "relayvm plugin: %s\n", err)
	return 1
}
