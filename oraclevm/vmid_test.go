...
import (
	...
	upstream "github.com/luxfi/oracle/vm"
	"github.com/luxfi/vm/chain"
)

// canonicalVMIDCB58 is O-Chain's vmID in the encoding the node's plugin
...
const canonicalVMIDCB58 = "r5m1ujrmXxVcQetG3CQfuDLHp2RHKh6vCDaFgBRQfUcTZh7eS"

// The three declarations of O-Chain's vmID must be one value.
//
// luxfi/oracle declares its own private literal, `ids.ID{'o','r','a','c','l',
// 'e','v','m'}` at vm/factory.go. luxfi/constants declares OracleVMID. They
// agree today and nothing in either repo makes them. This repo builds the
// plugin binary whose FILENAME is that id, so this is the layer where the
// agreement is checked; if luxfi/oracle's literal ever moves, a node would
// look for a plugin under one name while genesis named the other, and the
// chain would simply never appear.
func TestVMID_IsCanonicalAndStable(t *testing.T) {
	if VMID != constants.OracleVMID {
		t.Fatalf("VMID = %s, want constants.OracleVMID = %s", VMID, constants.OracleVMID)
	}
	if VMID != upstream.VMID {
		t.Fatalf("VMID = %s, but luxfi/oracle/vm.VMID = %s\n"+
			"The upstream private literal has drifted from constants.OracleVMID.\n"+
	...
	if got := VMID.String(); got != canonicalVMIDCB58 {
		t.Fatalf("VMID CB58 = %q, want %q\n"+
			...
			"  luxfi/constants   vm_ids.go OracleVMID\n"+
			"  luxfi/oracle      vm/factory.go VMID\n"+
			"  chains/oraclevm   oraclevm.go, this test",
...
func TestVMID_Bytes(t *testing.T) {
	want := ids.ID{'o', 'r', 'a', 'c', 'l', 'e', 'v', 'm'}
	if VMID != want {
		t.Fatalf("VMID bytes = %v, want %v", VMID[:10], want[:10])
	}
}

// O-Chain and R-Chain are different chains, so their ids are different values.
// The two shims are near-copies of each other; a copy that kept the wrong id
// would still compile, still pass a byte test if that were copied too, and
// produce two plugins racing for one filename.
func TestVMID_IsNotAnotherChains(t *testing.T) {
	for name, other := range map[string]ids.ID{
		...
		if VMID == other {
			t.Fatalf("oraclevm.VMID equals constants.%s (%s)", name, other)
		}
	}
}
...
// the value's real type is checked nowhere at compile time. oraclevm.go
// asserts *VM implements chain.ChainVM; this asserts the Factory actually
// produces a *VM. Together they are the whole claim, and the plugin needs no
...
func TestFactoryBuildsTheVMThePluginServes(t *testing.T) {
	...
	vm, ok := raw.(*VM)
	if !ok {
		t.Fatalf("Factory.New built a %T, want *oraclevm.VM", raw)
	}
	if vm == nil {
...
func TestTheVMReportsAVersion(t *testing.T) {
	...
	if want := upstream.Version.String(); v != want {
		t.Fatalf("VM version = %q, want luxfi/oracle/vm.Version = %q", v, want)
	}
}
