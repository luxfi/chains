...
//
// luxfi/oracle's own feegate test builds `&VM{feePolicy: fee.NoUserTxPolicy{}}`
...
func TestAVMBuiltThroughThisShimTakesNoUserTransactions(t *testing.T) {
	...
	if _, ok := policy.(fee.NoUserTxPolicy); !ok {
		t.Fatalf("FeePolicy() = %T, want fee.NoUserTxPolicy — O-Chain is service-only, "+
			"operator observations arrive through consensus and there is no user mempool", policy)
	}
	if err := fee.Validate(policy); err != nil {
		t.Fatalf("the node's boot gate rejects O-Chain's own declaration: %v", err)
	}
}
