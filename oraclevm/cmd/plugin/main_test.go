...
func TestVersionNamesTheIdTheBinaryMustBeInstalledAs(t *testing.T) {
	...
	got := out.String()
	if !strings.Contains(got, oraclevm.VMID.String()) {
...
// plugin used to print a hardcoded "Oracle-VM/1.0.0" while the VM reported its
// own number from luxfi/oracle; bumping either left the two disagreeing, and
// the one an operator reads is the plugin's.
...
func TestAPluginWithNoNodeToServeSaysSoAndFails(t *testing.T) {
	...
	if !strings.Contains(errOut.String(), "oraclevm plugin:") {
...
func TestEveryRefusalIsAttributedAndNonZero(t *testing.T) {
	...
	if got := w.String(); got != "oraclevm plugin: disk fell off\n" {
